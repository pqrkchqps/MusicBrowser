...
TEST_TARGETS = [ 'html', 'benchmark_smoke', 'json']

TEST_INFO = {
    'dartium': {
        'core': {
            # FIXME: test.dart does not yet support the combination of
            # --checked and --runtime=dartium.  For now, use checked
            # only with DRT.
            'unchecked': TEST_DARTIUM_FLAGS + TEST_TARGETS,
        },
    },
    'drt': {
        'layout': {
...
def main():
  ...
  build_root, drt_path, dartium_path, dart_path  = {
      'mac': (
        'out',
        os.path.join('DumpRenderTree.app', 'Contents', 'MacOS', 'DumpRenderTree'),
        os.path.join('Chromium.app', 'Contents', 'MacOS', 'Chromium'),
        'dart',
      ),
      'linux': ('out', 'DumpRenderTree', 'chrome', 'dart'),
      'win': ('build', 'DumpRenderTree.exe', 'chrome.exe', 'dart.exe'),
  }[HOST_OS]
      ...
      try:
        if ('layout' in suites and
          ...
          utils.runCommand(['python',
                            ...
                            '--additional-env-var', 'DART_FLAGS=%s' % dart_flags,
                            test])
